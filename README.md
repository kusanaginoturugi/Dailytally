...
送信フォームは既定で `https://tendo.net/advanced/app/doumu` を使います。`REPORT_ONLINE_FILE_FIELD` は未設定時 `up_file[]`、`REPORT_ONLINE_SUBMIT_FIELD` は未設定時 `kannondo` です。弥勒寺へ送る場合は `mirokuji` を設定してください。
